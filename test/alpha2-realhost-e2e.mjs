/**
 * 真实宿主行为 e2e：dsh v0.1.3-alpha.2 + dsh-smooth-scroll。
 *
 * 与 test/scroll-follow.test.mjs 的差别：后者在空白页里复刻宿主，本脚本驱动
 * 真实 `dsh web` 组合（profile web），用真实 LLM 流式输出走真实 ChatView
 * 跟随状态机（alpha.2 的贴底同步采样：`onScroll` 在贴底且非读者位移时立即
 * `sample()`，见 ChatView.tsx:604-616），断言插件在实际使用中的行为：
 * 追击、接管、导轨跳转、reduced-motion。
 *
 * 前置：
 *   - `dsh web --port <非3080> --no-open` 已启动，token 取自其 stdout；
 *   - profile web 已装本插件（link:）；工作区可缺省（回落到默认工作区）。
 *
 * 运行：node test/alpha2-realhost-e2e.mjs <token> [--port <端口>] [--workspace <名>]
 */
...
const port = process.argv.includes('--port') ? Number(process.argv[process.argv.indexOf('--port') + 1]) : 3416
const workspace = process.argv.includes('--workspace')
  ? process.argv[process.argv.indexOf('--workspace') + 1]
  : 'dsh-smooth-plugin'
const only = process.argv.includes('--only') ? process.argv[process.argv.indexOf('--only') + 1] : null
if (token === undefined) {
  console.error('usage: node alpha2-realhost-e2e.mjs <token> [--port <端口>] [--workspace <名>]')
  process.exit(2)
}

const PROMPT = (n) => `请只输出文本，不使用任何工具，不要输出代码块或列表：以《${n}》为题写一篇约1100字中文短文，逐自然段输出，段与段之间空一行。`
...
const INSTRUMENT = () => {
  ...
  // 真正承载会话内容的滚动容器是内容高度（scrollHeight）最大的那个；插件
  // 本身会对全部元素 attach，这里只取测量目标。
  let el = null
  let best = -1
  for (const c of document.querySelectorAll('[data-conversation-scroll]')) {
    if (c.scrollHeight > best) { best = c.scrollHeight; el = c }
  }
...
async function openApp(emulateReduced = false) {
  ...
  page.on('console', msg => {
    ...
    // 与本插件无关的宿主资源 404：alpha.2 的 ui-open-in-app 在 Windows 上
    // 取不到 Explorer 图标（/open-in-app/icon/explorer），不计入失败。
    if (loc?.url?.includes('/open-in-app/icon/')) return
    consoleErrors.push(`console.error: ${msg.text().slice(0, 300)}${loc?.url ? ` @ ${loc.url}` : ''}`)
  })
  page.on('pageerror', err => consoleErrors.push(`pageerror: ${String(err).slice(0, 300)}`))
  await page.goto(entryUrl(token, port), { waitUntil: 'domcontentloaded', timeout: 60000 })
...
async function sendPrompt(page, turn, waitMs = 150000) {
  ...
  // 先等 Hero 切到会话视图并挂载用户消息，再取增长基线：否则基线落在 Hero
  // 容器上，会话视图接管后的真实增长会被误判为「未增长」。
...
async function scenarioB(page) {
  ...
  // 等到流式产出足够可滚动内容（离底 > 600px）再滚轮。
  const deadline = Date.now() + 120000
  let info = null
  while (Date.now() < deadline) {
    info = await page.evaluate(INSTRUMENT)
    if (info !== null && info.floor > 600) break
  ...
  // 真实读者输入是持续多帧的偏离（README：连续 ≥2 帧才判定接管）。单次
  // mouse.wheel 只产生一帧位移，会走插件的「一次性非用户位移」吸收路径并
  // 继续跟随——那是设计行为。这里连续派发滚轮（相邻帧），模拟持续滚动。
  for (let i = 0; i < 16; i++) await page.mouse.wheel(0, -180)
  await page.waitForTimeout(250)
  const afterWheel1 = await page.evaluate(INSTRUMENT)
  // 等宿主完成脱钩判定（alpha.2 在读者位移后由 500ms 采样周期接管归属）。
  ...
  // 滚轮作用量取「手势刚结束」的采样：若接管失败，位置已被拉回，用稳点值
  // 会把失败误判成「滚轮未生效」而跳过断言。
  const wheelEffect = Math.max(0, beforeWheel.real - afterWheel1.real)
...
writeFileSync(new URL('./alpha2-e2e-report.json', import.meta.url), JSON.stringify(report, null, 2))
